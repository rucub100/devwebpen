...
public class ServerCertificateGenerator {
    ...
    /**
     * Constructs a {@code ServerCertificateGenerator} with the given
     * {@code KeyStore} and options.
     *
     * @param keyStore                  the {@code KeyStore} containing the root CA
     *                                  certificate.
     * @param serverCertificatesOptions the options to obtain the server
     *                                  configuration.
     * @throws NullPointerException if the given {@code KeyStore} does not contain a
     *                              certificate nor
     *                              the private key, and if the given options are
     *                              {@code null}.
     */
     ...
     * @return the {@code KeyStore} containing the certificate chain, server
     *         certificate and root CA
     *         certificate.
     * @throws GenerationException  if an error occurred while generating the
     *                              certificate.
    ...
    public synchronized KeyStore generate(CertData certData) {
        ...
        final CertConfig serverCertConfig = new CertConfig(Duration.ofDays(368));

        return cache.computeIfAbsent(
                certData,
                data -> CertificateUtils.createServerKeyStore(
                        rootCaCert,
                        rootCaPublicKey,
                        rooCaPrivateKey,
                        data,
